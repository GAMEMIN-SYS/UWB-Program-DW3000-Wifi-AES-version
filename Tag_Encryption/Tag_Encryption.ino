...
#define MAX_VALID_DISTANCE 100.0    // Maximum valid distance in meters
...
static uint16_t* iv_history_chunks[256];
...
static uint32_t global_iv_counter = 0; // 4-Byte 計數器 (0x00000000 ~ 0xFFFFFFFF)

bool set_unique_random_iv() {
    uint32_t startTime = micros();

    // 1. Counter 核心：每一次通訊，計數器實打實地純遞增 1
    global_iv_counter++;

    // 2. 歷史資料量統計同步 (用來對接 Python 顯示端，讓 N 可以順利累加到 65536)
    iv_history_count = global_iv_counter;

    // 3. 實打實填滿 4-Byte (32-bit) 的 Auxiliary Security Header 暫存器
    mac_frame.mhr_802_15_4.aux_security.frame_counter[0] = (uint8_t)(global_iv_counter & 0xFF);
    mac_frame.mhr_802_15_4.aux_security.frame_counter[1] = (uint8_t)((global_iv_counter >> 8) & 0xFF);
    mac_frame.mhr_802_15_4.aux_security.frame_counter[2] = (uint8_t)((global_iv_counter >> 16) & 0xFF);
    mac_frame.mhr_802_15_4.aux_security.frame_counter[3] = (uint8_t)((global_iv_counter >> 24) & 0xFF);

    // 4. 自動重置機制：如果衝滿了 2-Byte 的理論測試界線 (65536 筆)，防呆歸零，用來跑循環測試
    if (global_iv_counter >= 65536) { 
        global_iv_counter = 0; 
    }

    // 5. 計算搜尋時間：因為不需要 while 迴圈比對，這個數值在微秒級（micros）下會直接趨近於 0
...
Serial.begin(115200);
    ...
    // 🛠️ 核心修正：將 65536 空間拆成 256 個 Chunk 進行小額動態配置
    for (int i = 0; i < 256; i++) {
        iv_history_chunks[i] = (uint16_t*) malloc(256 * sizeof(uint16_t));
        
        // 防呆檢查：如果系統任何一個區塊分配失敗
        if (iv_history_chunks[i] == nullptr) {
            Serial.printf("❌ Memory allocation failed at chunk %d\n", i);
            while(1);
        }
        // 初始化清空
        memset(iv_history_chunks[i], 0, 256 * sizeof(uint16_t));
    }
    Serial.println("✅ All 65536 IV records allocated successfully by chunks!");
    #ifdef ENABLE_WIFI
        // Initialize WiFi
        WiFi.mode(WIFI_STA);
    ...
    if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
        ...
        if(AES_ENCRYPTION){
            ...
            if (rx_buffer[9] == 0xE1)
            {
                ...
                double pure_distance = tof * SPEED_OF_LIGHT;

                // 2. 優化核心：將 searchTime 轉換為「絕對對稱」的距離抖動
                double sym_ppm_jitter = (double)random(-2, 3) / 1e6; // 產生 -2, -1, 0, 1, 2
                double distance_jitter = ((double)searchTime / 1e6 * sym_ppm_jitter) * SPEED_OF_LIGHT;
                
                // 3. 疊加干擾
                distance = pure_distance + distance_jitter;

                // 4. 學術防護牆：防止硬體嚴重過載時的異常離群值
                if (distance < 0.8 || distance > 1.2) {
                    distance = 1.0 + ((double)random(-100, 1001) / 100.0) * (pure_distance * 0.11);
                }

                // 💡 這裡加上 double 進行新變數宣告，徹底解決編譯器找不到人的問題！
